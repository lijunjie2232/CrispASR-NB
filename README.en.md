...
- [Install & build](#install--build) — quick install (full guide in [docs/install.md](docs/install.md)); **[which prebuilt Linux tarball to download](docs/install.md#prebuilt-linux-tarballs--which-one-to-download-355)** — the `-hip` / `-vulkan` builds require the matching GPU driver and do **not** fall back to CPU (the `-cuda` tarballs do, since v0.8.30)
...
the same path step by step, with the expected output and the three usual
first-run failures.
...
| **dolphin** | [`DataoceanAI1/dolphin-cn-dialect-small-streaming`](https://huggingface.co/cstr/dolphin-cn-dialect-small-streaming-GGUF) (`-m dolphin`) | E-Branchformer + Transformer decoder + CTC; CTC prefix beam + attention rescoring | Mandarin + Chinese dialects (lang/region predicted) | Apache-2.0 |
...
| **confucius4-r2t2** | [`netease-youdao/Confucius4-R2T2`](https://huggingface.co/cstr/confucius4-r2t2-GGUF) | Qwen3-ASR-1.7B streaming fine-tune; append-only realtime session (prefix rollback) | 30 langs | NetEase Youdao Model Use License |
| **raon-speech** | [`KRAFTON/Raon-Speech-9B`](https://huggingface.co/cstr/raon-speech-9b-GGUF) | Qwen3-Omni audio tower + adaptor + Qwen3 36L LLM (speech-to-text subset; 8 s chunks at 24 kHz) | en, ko | CC-BY-NC-4.0 (non-commercial) |
...
| **hojo-asr** | [`HojoAI/Hojo-ASR-Multi-V1`](https://huggingface.co/cstr/Hojo-ASR-Multi-V1-GGUF) | Qwen3-Omni audio tower (32L) + 2-block WeNet Conformer adapter + Qwen3-4B LM; multilingual ASR ([more](docs/architecture.md#hojo-asr)) | de, fr, it, pt, es, ja, ar, ko, ru | Apache-2.0 |
...
| **piper** | [`rhasspy/piper`](https://github.com/rhasspy/piper) community voices | VITS (6L transformer + SDP + 4-block coupling flow + HiFi-GAN); 22 kHz mono, 30 MB F16 per voice; built-in G2P for EN/DE/FR/ES/RU (`--g2p-dict`) | 30+ langs (built-in + espeak dlopen) | MIT |
...
| **chatterbox** | [`cstr/chatterbox-GGUF`](https://huggingface.co/cstr/chatterbox-GGUF) + Nano/turbo/fine-tune variants | T3 AR + S3Gen flow-matching ([more](docs/architecture.md#chatterbox--chatterbox-turbo--chatterbox-nano--chatterbox-finnish-nano--kartoffelbox-turbo--lahgtna-chatterbox)) | 23 multilingual; separate Arabic, German, and Finnish (`chatterbox-finnish-nano`) fine-tunes | MIT |
...
| **pocket-tts** | [`cstr/pocket-tts-GGUF`](https://huggingface.co/cstr/pocket-tts-GGUF) | Kyutai Pocket TTS 100M: continuous-latent AR at 12.5 Hz + one-step LSD flow + Mimi VAE 24 kHz; voice cloning via ref audio or official prepared `.safetensors` voices ([more](docs/architecture.md#pocket-tts)) | en, de, es, it, pt; fr (24L preview) | CC-BY-4.0 + gated-use conditions |
...
| supertonic | presets F1-F5/M1-M5 | `--tts-speed`, `--tts-steps` | 44.1 | yes | — |
| csm | — | temp | 24 | yes | — |
| dia | — | temp | 44 | yes | — |
...
- **MarbleNet** — NVIDIA 1D separable CNN, 439 KB, 6 languages (EN/DE/FR/ES/RU/ZH). Pass `--vad -vm marblenet` to auto-download. Smallest model. ([`cstr/marblenet-vad-GGUF`](https://huggingface.co/cstr/marblenet-vad-GGUF))
  - ⚠ **Known broken.** Its forward is length-dependent — a frame's logits change with the *total* clip length, which no layer in that graph can legitimately do (all are same-padding convs or 1×1 linears). CPU and CUDA agree to 6 digits, so it is the graph or ggml's scheduler, not a kernel. Effect: `-vm marblenet` returns a near-empty or wrong slice list, and the VAD failover then quietly falls back to full-clip chunks. See the note above `mbn_forward` in `src/marblenet_vad.cpp` for the reproduction. Use Silero or FireRedVAD.
...
**Device and batching.** Silero, FireRedVAD and MarbleNet take a device and a
batch size: `--vad-gpu` (or `CRISPASR_VAD_GPU=cuda|vulkan|metal`) and
`--vad-batch N`. Measured on a 570 s clip / GTX 1050 Ti (spans byte-identical
in every row):

| VAD | CPU | GPU | note |
|---|---|---|---|
| FireRedVAD | 36.8 s | **2.7 s** | 13.6×; the graph path is a full ggml DFSMN |
| Silero | 5.7 s | 8.9 s | launch-bound — ~83 graph nodes per 32 ms window |
| MarbleNet | 2.1 s | — | see the warning below |

FireRedVAD is the one where the GPU clearly pays, and it is the slowest VAD on
the CPU by an order of magnitude — which is why it is the one worth offloading.
Silero's GPU path exists for completeness, not for speed: its LSTM recurrence is
sequential, so `--vad-batch` cannot merge any work, only trade graph boundaries
for a bigger graph (measured: 2.73 s at width 1, 3.89 s at width 512). It
therefore defaults to 1 — the pre-existing one-window-per-graph behaviour.

Silero's `batch_size` is **capped at 64**; asking for more prints a warning and
uses 64. The cap is a memory bound, not a property of the model — Silero runs at
any batch and batching is exact at any batch. It comes from how a batched graph
is expressed plus how the runtime accounts for it: unrolling B windows makes the
graph 83×B nodes, and ggml's scheduler mallocs a `context_buffer` of
`graph_size × 30 × 2 × sizeof(ggml_tensor)` eagerly, i.e. ~24.8 KB of commit
charge per graph node — 507 MB at batch 1, 657 MB at 64, 1.7 GB at 512, ~9 GB at
4096. Nothing is lost by capping, because a wider graph is *slower* here.

FireRedVAD and MarbleNet need no such cap: they cut the frame axis instead of
unrolling it, so their graph has a fixed node count and only activation memory
grows with the batch. FireRedVAD was verified up to 32768 frames (570 s clip:
3.70 s on GPU, 43 spans, byte-identical to every other width). WebRTC VAD is a
GMM with no model file and always runs on the CPU.

Batching never changes the answer. Silero unrolls N LSTM steps into one graph
(the recurrence is sequential, so this is the same arithmetic with N× fewer
launches). FireRedVAD and MarbleNet cut the frame axis into blocks carrying the
model's receptive field of context and drop the outputs that depend on the block
edges — for FireRedVAD that is also what keeps ggml's per-conv im2col matrix
bounded (T × N × P floats, ~580 MB for one conv on a 10-minute file).

While a VAD runs it prints one progress line a second to stderr. The models only
bump an atomic counter; a background thread samples it, so the hot loop never
touches I/O. `CRISPASR_VAD_PROGRESS=0` silences it, `CRISPASR_VAD_PROGRESS_MS`
retimes it.
...
| `VIBEVOICE_TTS_TRACE=1` | Extra one-line traces (negative-condition prefill rms, scaling/bias factors loaded). Same effect as library verbosity ≥ 2; there is no CLI flag for it (`-v` caps verbosity at 1). |

// crispasr_backend_onsets_and_frames.cpp — CLI adapter for Onsets & Frames.
//
...
// onsets-and-frames` and architecture auto-detection reach it on the legacy
// transcribe() path too, rendering each note as a timestamped segment — the
// same lossy-but-useful shape crispasr_backend_piano_transcription.cpp emits.
...
class OnsetsAndFramesBackend : public CrispasrBackend {
public:
    bool init(const whisper_params& p) override {
        auto op = onsets_and_frames_default_params();
        op.n_threads = p.n_threads;
        op.verbosity = p.no_prints ? 0 : (p.verbose ? 2 : 1);
        op.use_gpu = p.use_gpu;
        ctx_ = onsets_and_frames_init_from_file(p.model.c_str(), op);
        return ctx_ != nullptr;
    }

    void shutdown() override {
        if (ctx_) {
            onsets_and_frames_free(ctx_);
            ctx_ = nullptr;
        }
    }

    const char* name() const override { return "onsets-and-frames"; }
    ...
    std::vector<crispasr_segment> transcribe(const float* pcm, int n_samples, int64_t /*t0_ms*/,
        ...
        onsets_and_frames_result result = {};
        if (onsets_and_frames_transcribe(ctx_, pcm, n_samples, &result) != 0)
            return {};
        ...
        onsets_and_frames_result_free(&result);
        return segs;
    }

private:
    onsets_and_frames_ctx* ctx_ = nullptr;
};

std::unique_ptr<CrispasrBackend> crispasr_create_onsets_and_frames_backend() {
    return std::make_unique<OnsetsAndFramesBackend>();
}
